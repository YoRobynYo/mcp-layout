...
if (typeof PanelManager === 'undefined') {
    class PanelManager {
        ...
        init() {
            ...
            // Initialize cube face click listeners - THIS IS THE MISSING PIECE
            this.initializeCubeFaceListeners();
        }

        // Initialize cube face click listeners - NEW METHOD
        initializeCubeFaceListeners() {
            console.log('Initializing cube face click listeners...');
            
            // Wait for cubes to be fully loaded in the DOM
            setTimeout(() => {
                this.attachFaceListeners('cube-1');
                this.attachFaceListeners('cube-2');
            }, 2000); // Give time for cube draggers to initialize
        }

        // Attach click listeners to cube faces - NEW METHOD
        attachFaceListeners(cubeId) {
            const cube = document.getElementById(cubeId);
            if (!cube) {
                console.warn(`Cube ${cubeId} not found in DOM`);
                return;
            }
            
            // Find faces using multiple selectors to catch different HTML structures
            const faces = cube.querySelectorAll('.face, .cube-face, div[class*="face"]');
            console.log(`Found ${faces.length} faces for ${cubeId}`);
            ...
            faces.forEach((face, index) => {
                ...
                face.addEventListener('click', (e) => {
                    e.stopPropagation();
                    e.preventDefault();
                    console.log(`Face clicked: ${cubeId} - Face ${index + 1}`);
                    this.handleFaceClick(cubeId, index + 1);
                });
                ...
                face.addEventListener('mouseenter', () => {
                    ...
                    face.style.transition = 'opacity 0.2s ease';
                });
            ...
            console.log(`Attached click listeners to ${faces.length} faces of ${cubeId}`);
        }

        // Handle cube face clicks - NEW METHOD
        handleFaceClick(cubeId, faceIndex) {
            console.log(`Processing face click: ${cubeId} - Face ${faceIndex}`);
            
            // Update selected cube to match the clicked cube
            if (cubeId === 'cube-1') {
                this.selectedCube = 'cube-1';
                this.updateScreenButtonState('1');
            } else if (cubeId === 'cube-2') {
                this.selectedCube = 'cube-2';
                this.updateScreenButtonState('2');
            }
            
            // Route to appropriate face handler
            if (cubeId === 'cube-1') {
                this.handleCube1Face(faceIndex);
            } else if (cubeId === 'cube-2') {
                this.handleCube2Face(faceIndex);
            }
        }

        // this part shows when I click face 1 and it shouldn't be here
        // and face 3 still isn't opening a youtube downloader 
        // Handle Cube-1 face clicks - NEW METHOD
        handleCube1Face(faceIndex) {
            console.log(`Handling Cube-1 Face ${faceIndex}`);
            
            switch(faceIndex) {
                case 1:
                    console.log('Face 1: Opening Application Menu');
                    this.openElectronApp('openApplicationMenu', 'Application Menu');
                    break;
                case 2:
                    console.log('Face 2: Opening File Manager');
                    this.openElectronApp('openFileManager', 'File Manager');
                    break;
                case 3:
                    // THIS FIXES YOUR FACE 3 ISSUE
                    console.log('Face 3: Opening System Monitor');
                    this.openSystemMonitorFace3();
                    break;
                case 4:
                    console.log('Face 4: Opening Network Settings');
                    this.openElectronApp('openNetworkSettings', 'Network Settings');
                    break;
                case 5:
                    console.log('Face 5: Opening Terminal');
                    this.openElectronApp('openTerminal', 'Terminal');
                    break;
                case 6:
                    console.log('Face 6: Opening Settings');
                    this.openElectronApp('openSettings', 'Settings');
                    break;
                default:
                    console.log(`Unknown face: ${faceIndex}`);
            }
        }

        // Handle Cube-2 face clicks - NEW METHOD
        handleCube2Face(faceIndex) {
            console.log(`Handling Cube-2 Face ${faceIndex}`);
            
            switch(faceIndex) {
                case 1:
                    console.log('Face 1: Opening Development Tools');
                    this.openElectronApp('openDevTools', 'Development Tools');
                    break;
                case 2:
                    console.log('Face 2: Opening Code Editor');
                    this.openElectronApp('openCodeEditor', 'Code Editor');
                    break;
                case 3:
                    console.log('Face 3: Opening Database Manager');
                    this.openElectronApp('openDatabaseManager', 'Database Manager');
                    break;
                case 4:
                    console.log('Face 4: Opening API Tester');
                    this.openElectronApp('openApiTester', 'API Tester');
                    break;
                case 5:
                    console.log('Face 5: Opening Log Viewer');
                    this.openElectronApp('openLogViewer', 'Log Viewer');
                    break;
                case 6:
                    console.log('Face 6: Opening Project Manager');
                    this.openElectronApp('openProjectManager', 'Project Manager');
                    break;
                default:
                    console.log(`Unknown face: ${faceIndex}`);
            }
        ...
        // THE KEY METHOD for Face 3 - NEW METHOD
        openSystemMonitorFace3() {
            console.log('Opening System Monitor from Face 3...');
            
            // Try the specific Face 3 API first
            if (window.electronAPI && window.electronAPI.openSystemMonitorFace3) {
                console.log('Calling electronAPI.openSystemMonitorFace3()');
                try {
                    window.electronAPI.openSystemMonitorFace3();
                    return;
                } catch (error) {
                    console.error('Error calling openSystemMonitorFace3:', error);
                }
            }
            
            // Fallback to generic system monitor
            if (window.electronAPI && window.electronAPI.openSystemMonitor) {
                console.log('Fallback: calling electronAPI.openSystemMonitor()');
                try {
                    window.electronAPI.openSystemMonitor();
                    return;
                } catch (error) {
                    console.error('Error calling openSystemMonitor:', error);
                }
            }
            
            // Final fallback
            console.warn('No electronAPI available for System Monitor');
            alert('System Monitor would open here!\n(Face 3 clicked - electronAPI not available)');
        }

        // Generic Electron app opener - NEW METHOD
        openElectronApp(apiMethod, appName) {
            if (window.electronAPI && window.electronAPI[apiMethod]) {
                console.log(`Opening ${appName} via ${apiMethod}`);
                try {
                    window.electronAPI[apiMethod]();
                } catch (error) {
                    console.error(`Error calling ${apiMethod}:`, error);
                }
            } else {
                console.warn(`electronAPI.${apiMethod} not available`);
                alert(`${appName} would open here!\n(${apiMethod} not available)`);
            }
        }

        // Update screen button visual state - NEW METHOD
        updateScreenButtonState(screen) {
            const screenBtns = document.querySelectorAll('.screen-btn');
            screenBtns.forEach(btn => {
                const isActive = btn.dataset.screen === screen;
                btn.classList.toggle('active', isActive);
                
                // Apply your existing button styling
            ...
            console.log(`Updated screen button state to: ${screen}`);
        }
...
// // Handles button event initialization and setup
